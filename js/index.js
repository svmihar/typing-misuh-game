...
var Component = function (_React$Component) {
  ...
  function Component(props) {
    ...
    _this.getWordList = _this.getWordList.bind(_this);
    _this.getRandomInt = _this.getRandomInt.bind(_this);
    _this.getWord = _this.getWord.bind(_this);
    _this.checkEqual = _this.checkEqual.bind(_this);
    _this.timer = _this.timer.bind(_this);
    _this.startGame = _this.startGame.bind(_this);
    _this.rating = _this.rating.bind(_this);
    _this.switchFonts = _this.switchFonts.bind(_this);
    _this.interval;
    return _this;
  }
  ...
  _createClass(Component, [{
    ...
    value: function render() {
      ...
      var letters = [];
      var board = void 0;
      this.state.activeWord.map(function (current, index) {
        var correct = void 0;
        ...
        letters.push(React.createElement(
          'span',
          { className: 'game-letter', key: index, 'data-correct': correct },
          current
        ));
      });
      if (!this.state.gameStarted) {
        board = React.createElement(
          'div',
          { className: 'game__board', key: 'start' },
          React.createElement(
            'p',
            null,
          ...
          React.createElement(
            'button',
            { className: 'button', onClick: this.startGame },
            'Start'
          ),
          React.createElement(
            'a',
            { href: '#', onClick: this.switchFonts },
            this.state.font === 'sans' ? 'Serif font please' : 'Sans font please'
          )
        );
      } else if (this.state.timer && this.state.gameStarted) {
        board = React.createElement(
          'div',
          { className: 'game__board', key: 'inprogress' },
          React.createElement(
            'div',
            { className: 'game__score' },
            'SCORE: ' + this.state.wordsMastered
          ),
          React.createElement(
            ...
            React.createElement(
              'div',
              { className: 'game__words', key: this.state.activeWord },
              letters
            )
          ),
          React.createElement(
        ...
        board = React.createElement(
          'div',
          { className: 'game__board', key: 'timesup' },
          React.createElement(
            'div',
            ...
            React.createElement(
              'p',
              null,
              'FINAL SCORE: ' + this.state.wordsMastered,
              React.createElement(
                'span',
                { className: 'emoji' },
                this.rating()
